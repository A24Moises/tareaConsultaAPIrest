...
fun main() {
    ...
    if (response.statusCode() == 200) {
        val countries = Json.decodeFromString<List<Country>>(response.body())

        // 1️⃣ Países ordenados por población de mayor a menor
        countries
            .sortedByDescending { it.population }
            .take(10)
            .forEach { country ->

        // 2️⃣ Agrupar países por continente
        println("\n🌍 Países agrupados por continente:")
        countries
            .groupBy({ it.continents.firstOrNull() ?: "Desconocido" }, { it })
            .forEach { (continent, countryList) ->
            println("🔹 $continent: ${countryList.size} países") }

        // 3️⃣ Países con más de 50 millones de habitantes
        ...
        // 4️⃣ Lista de idiomas únicos en el mundo
        println("\n🗣️ Lista de idiomas únicos hablados:")
        val uniqueLanguages = countries
                                    .flatMap { it.languages?.values ?: emptyList() }
                                    .distinct()
        println(uniqueLanguages
                            .joinToString(", "))

        // 5️⃣ Población total del planeta
        val totalPopulation = countries
                                    .sumOf<Country> { it.population }
